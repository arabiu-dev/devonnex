...
async function getData(postId) {
  const res = await fetch(`http://localhost:3030/api/v1/posts/${postId}`, {
    cache: "no-store",
  });
...
async function PostsShowPage({ searchParams: { postId } }) {
  ...
  const addComment = async (formData) => {
    ...
    const res = await fetch(`http://localhost:3030/api/v1/comments/`, {
      method: "post",
      headers: {
              ...
              {post.user_id === id && (
                <Ellipses
                  handlers={[
                    ...
                    [
                      "Delete",
                      `http://localhost:3030/api/v1/posts/${postId}`,
